...
namespace Wave.XR.Sample.Controller
{
	...
	{
		const string LOG_TAG = "Wave.XR.Sample.Controller.HMDTracking";
		...
		private const InputDeviceCharacteristics kHMDCharacteristics = (
		...
		private bool GetPosition(out Vector3 position, out string msg)
		{
			...
			foreach (UnityEngine.XR.InputDevice id in m_InputDevices)
			{
				...
				if (id.characteristics.Equals(kHMDCharacteristics))
				{
					if (id.TryGetFeatureValue(UnityEngine.XR.CommonUsages.centerEyePosition, out Vector3 value))
					{
						position = value;
						return true;
					}
					...
					{
						msg = "Get CommonUsages.centerEyePosition failed.";
					}
				}
				else
				{
		...
		private bool GetRotation(out Quaternion rotation, out string msg)
		{
			...
			foreach (UnityEngine.XR.InputDevice id in m_InputDevices)
			{
				...
				if (id.characteristics.Equals(kHMDCharacteristics))
				{
					if (id.TryGetFeatureValue(UnityEngine.XR.CommonUsages.centerEyeRotation, out Quaternion value))
					{
						rotation = value;
						return true;
					}
					...
					{
						msg = "Get CommonUsages.centerEyeRotation failed.";
					}
				}
				else
				{
