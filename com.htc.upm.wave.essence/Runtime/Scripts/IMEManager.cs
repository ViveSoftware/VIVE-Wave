...
namespace Wave.Essence
{
	public class IMEManager
	{
		...
		public void showKeyboard(IMEParameter parameter, bool isEnableEditorPanel, inputCompleteCallback cb, inputClickedCallback clickedCallback)
		{
			...
			imeManager.Call("showKeyboard", javaIMEParameterFromCS(parameter), new RequestCompleteHandler(), isEnableEditorPanel);
		}
		...
		public class InputResult
		{
            ...
            public InputResult(int id, string content, int keyCode, int errorCode)
            {
                mId = id;
                mContent = content;
                mErrorCode = errorCode;
                Log.d(LOG_TAG, "tay key = " + (Key)keyCode);
		...
		class RequestCompleteHandler : AndroidJavaProxy
		{
			...
			public void onInputClickedwithObject(AndroidJavaObject resultObject)
			{
				Log.i(LOG_TAG, "unity clicked callback with result object");
				if (mClickedCallback == null)
				{
					Log.w(LOG_TAG, "unity callback but user clicked callback is null ");
				}

				int id = resultObject.Get<int>("id");
				int errorCode = resultObject.Get<int>("errorCode");
				string inputContent = resultObject.Get<string>("inputContent");
				int keyCode = resultObject.Get<int>("keyCode");

				InputResult inputResult = new InputResult(id, inputContent, keyCode, errorCode);
