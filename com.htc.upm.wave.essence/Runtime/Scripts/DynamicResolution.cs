...
namespace Wave.Essence.Render
{
    ...
    public class DynamicResolution : MonoBehaviour
    {
        ...
        private bool isInitRunning = false;
        ...
        // An issue is eyeTextureWidth and Height are 0 when the application is launched.
        // To avoid this issue, we need to wait until eyeTextureWidth and Height are set.
        IEnumerator Init()
        {
            Log.d(LOG_TAG, "Init", true);
            if (isInitialized || isInitRunning) yield break;

            isInitRunning = true;

            if (XRSettings.eyeTextureWidth <= 0 || XRSettings.eyeTextureHeight <= 0)
            {
                Log.d(LOG_TAG, "Waiting for XRSettings.eyeTextureWidth and XRSettings.eyeTextureHeight to be set", true);
                yield return new WaitUntil(() => XRSettings.eyeTextureWidth > 0 && XRSettings.eyeTextureHeight > 0);
            }
            ...
            Log.d(LOG_TAG, "Finalized Resolution Scale List: " + string.Join(",", resolutionScaleList.ToArray()), true);
        ...
        {
            // Set hard limit to avoid Unity problem.
            rs = Mathf.Clamp(rs, 0.005f, 4);
            Log.d(LOG_TAG, $"SetResolutionScale({rs})");
        ...
        private float GetResScaleFromDMM()
        {
            ...
            Log.d(LOG_TAG, "Eye Buffer Width: " + halfWidth * 2 + " Eye Buffer Height: " + halfHeight * 2);
